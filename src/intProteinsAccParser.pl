...
for(my $i=1;$i<scalar(@intlines);$i++){
	...
	if(! defined($visitedProt{$p1File})){
		printAll($p1File, \%proteins, $fields[$hcol{"PROT1"}], \%hcol, \@fields);
		$visitedProt{$p1File}=1;
	}
	if(! defined($visitedProt{$p2File})){
		printAll($p2File, \%proteins, $fields[$hcol{"PROT2"}], \%hcol, \@fields);
		$visitedProt{$p2File}=1;
	}		
}
...
sub getAccessibilities{
	...
	foreach my $pos (@{${$protein{$chain}}{"positions"}}){
		my $acc = ${${$protein{$chain}}{"relAcc"}}{$pos};
		$accs{$pos}=$acc;
	}
	return(\%accs);
}
...
sub printAll{
	...
	if(defined($proteins{$p1File})){
		my %p1protein = %{$proteins{$p1File}}; 
		#Sequence including fragments obtained from the PDB
		my %pdbSequenceObj = %{getSequence(\%p1protein)};
		#Get accessibilities
		my %accessibilities = %{getAccessibilities(\%p1protein)};			
